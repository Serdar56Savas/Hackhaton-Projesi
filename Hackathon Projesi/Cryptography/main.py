...
import os
import pyodbc #mssql kütüphanesinin import edilmesi
...
class dilKontrol:
    ...
    def kelimeAyir(self,word):
        Kelimeler = word.split(" ")#metni boşluktan bölerek kelimelere ayrılmasının sağlanması
        return len(Kelimeler)

    def CumleAyir(self,word):
        Cumleler = word.split(".")#metni '.' dan bölerek cümlelere ayrılmasının sağlanması 
        return len(Cumleler)
        
    def sesliHarf(self,word):#sesli harf sayısını hesaplıyor.
        sayac = 0
        for i in word.lower():
    ...
    def BuyukUnluUyumu(self,word):
        ...
        for x in Kelimeler:
            if (sum(x.count(kalin) for kalin in self.kalinunluler)) != 0 and (sum(x.count(ince) for ince in self.inceunluler)) != 0: #kalın ve ince harflerin aynı anda var olması durumda büyük ünlü uyumuna uymaz 
                uymayanlar +=1 
            else:
                uyanlar += 1
...
#dosyadan metni okuyoruz 
try:
    dosya = open("sifrelenmemisVeri.txt", "r",encoding="utf-8")
    word = dosya.read()
except IOError:
...
try:
    conn = pyodbc.connect('Driver={SQL Server};'
                        'Server=LAPTOP-7M06I3FK\SQLEXPRESS;'
                        'Database=dbTicarii;'
    ...
    cursor = conn.cursor()
    com = cursor.execute("INSERT INTO dbo.sifreleme (sha3_512,sha2_512,blake2,md5,sha1,cipher,rsa) VALUES (?,?,?,?,?,?,?)",(sfr.sifreSha3_512(word),sfr.sifreSha2_512(word),sfr.sifreBlake2b(word),sfr.sifreMD5(word),sfr.sifreSHA1(word),0,sfr.sifreasymmetric(word)))
    com.commit()
finally: 
    conn.close()